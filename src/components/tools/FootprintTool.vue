<template>
  <div class="footprint-tool animate-fade-in">
    <!-- 工具顶部导航栏 -->
    <div class="tool-header-bar mb-20">
      <el-button class="btn-back-hub" @click="$emit('back')" size="default">
        <el-icon><Back /></el-icon>返回万能工具箱
      </el-button>
      <div class="tool-header-title">
        <span class="tool-badge-pill">万能工具箱 · 生活见证</span>
        <h2 class="section-title">幸福足迹</h2>
      </div>
    </div>

    <p class="section-subtitle mb-20">家庭旅行 · 地区点亮 · 幸福与足迹双重见证</p>
...
const groupedFootprints = computed(() => {
  ...
  provinceNames.forEach(prov => {
    groups[prov] = { name: prov, visited: false, cities: [] }
  })
...
<style scoped>
.footprint-tool { padding: 0; margin: 0 auto; }
...
.main-layout-row { margin-bottom: 20px; }
.layout-col { margin-bottom: 24px; }
...
.stats-badge strong { color: #6366f1; font-size: 14px; }
...
.prov-count-tag { font-weight: 600; }
.delete-prov-btn { font-size: 12px; }
...
.close-city-icon:hover { color: #ef4444; }
...
.all-provinces-scroll {
  display: flex;
  flex-direction: column;
  gap: 16px;
}
.region-group { display: flex; flex-direction: column; gap: 8px; }
.region-title {
  font-size: 13px;
  font-weight: 700;
  color: #64748b;
...
.tile-check-icon { color: #6366f1; font-size: 14px; }
.tile-plus-icon { color: #94a3b8; font-size: 12px; }
.tile-bottom {
  font-size: 11px;
}
.tile-desc.active {
